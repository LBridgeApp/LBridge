...
public class PatchUID {
    ...
    public static byte[] encodeSerialNumber(String serialNumber) {
        ...
        Log.v(TAG, String.format("1. Serial number: %s%n", serialNumber));

        char[] symbols = serialNumber.toCharArray();

        Log.v(TAG, String.format("2. Char array: %s%n", Arrays.toString(symbols)));
        ...
        Log.v(TAG, String.format("3. uuidShort: %s%n", Arrays.toString(uuidShort)));
        Log.v(TAG, String.format("3. uuidShort: %s%n", byteArrayToHex(uuidShort)));
        ...
        Log.v(TAG, String.format("4. serialBuffer: %s%n", Arrays.toString(serialBuffer)));
        Log.v(TAG, String.format("4. serialBuffer: %s%n", byteArrayToHex(serialBuffer)));

        byte[] patchUID = new byte[8];

        System.arraycopy(serialBuffer, 3, patchUID, 0, 8);

        Log.v(TAG, String.format("5. PatchUID: %s%n", Arrays.toString(patchUID)));
        Log.v(TAG, String.format("5. PatchUID: %s%n", byteArrayToHex(patchUID)));
